    ...
    update() { // must fix
        ...
        this.game.entities.forEach(function (entity) {
            if (entity.BB && that.BB.collide(entity.BB)) {
                if (entity instanceof ground || entity instanceof Float) {
                    if (that.lastBB.bottom <= entity.BB.top) {
                        that.y = entity.BB.top - PARAMS.BLOCKWIDTH;
                        that.velocity.y = 0;
                        if (that.state != 0) that.state = 0
                        that.updateBB();
                    } if ((entity instanceof ground || entity instanceof Wall && entity.type) && that.BB.collide(entity.BB)) {
